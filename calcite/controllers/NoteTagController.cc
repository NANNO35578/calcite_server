...
void NoteTagController::bindTags(const HttpRequestPtr &req, std::function<void(const HttpResponsePtr &)> &&callback) {
  verifyTokenAndGetUserId(req, [this, req, callback](bool valid, int64_t userId) {
    ...
    [this, noteId, tagIds, tagStr, userId, callback](size_t count) {
      // 如果tagIds为空，表示清除所有标签
      if (tagIds.empty()) {
      ...
      std::string sql = "SELECT user_id FROM calcite.tag WHERE user_id =" + std::to_string(userId) +
                        " AND id IN (" + tagStr +
      ...
      client->execSqlAsync(sql, [this, noteId, tagIds, callback](const drogon::orm::Result &result) {
        ...
        std::cout << "tag check passed, inserting note tags" << std::endl;
        ...
        clientToInsert->execSqlAsync(insertSQL, [this, callback](const drogon::orm::Result &result) {
